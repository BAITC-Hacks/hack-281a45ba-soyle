...
export default async function CatalogPage({ searchParams }: Props) {
  ...
  return <div><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Для команд</p><h1 className="page-title mt-2">Каталог задач</h1><p className="mt-2 text-slate-600">Все опубликованные задачи, включая те, которым ещё нужны уточнения.</p></div><span className="text-sm font-semibold text-slate-500">Найдено: {tasks.length}</span></div>
    <form className="panel mt-7 grid gap-4 md:grid-cols-3" action="/catalog" method="get">
      ...
      <div><label className="label" htmlFor="sort">Сортировка</label><div className="flex flex-col gap-2 sm:flex-row"><select className="input min-w-0" id="sort" name="sort" defaultValue={sort}><option value="score_desc">Сначала высокий рейтинг</option><option value="score_asc">Сначала низкий рейтинг</option><option value="newest">Сначала новые</option><option value="oldest">Сначала старые</option></select><button className="button-primary w-full sm:w-auto" type="submit">Применить</button></div></div>
    </form>
